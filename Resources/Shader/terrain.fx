...
PatchTess ConstantHS(InputPatch<VS_OUT, 3> input, int patchID : SV_PrimitiveID)
{
    ...
    //카메라의 거리에 따라서 테셀레이션 레벨이 달라진다.
    float minDistance = g_vec2_1.x;//테셀레이션이 적용되는 최소,최대 거리
    ...
    //카메라과 해당 엣지의 거리에 따라서 테셀레이션 레벨을 구해주는 사용자 정의 함수
...
// g_int_1      : TileX
// g_int_2      : TileZ
// g_float_0    : Max Tessellation Level //최대 테셀레이션
// g_vec2_0     : HeightMap Resolution //해상도
...
//여기서 원래 vertex shader에서 하던 좌표계산을 대신해서 해주게 된다.
struct DS_OUT
{
...
DS_OUT DS_Main(const OutputPatch<HS_OUT, 3> input, float3 location : SV_DomainLocation, PatchTess patch)
{
    ...
    //만들어진 터레인 메쉬의 UV 좌표는 0에서 부터 시작해서 1씩 쭉 증가하는 형식으로 되어 있다
    //그걸 0~1값으로 만들어 주기 위해 다시 계산해서 넣어준다.
    float2 fullUV = float2(uv.x / (float)tileCountX, uv.y / (float)tileCountZ);
    float height = g_tex_2.SampleLevel(g_sam_0, fullUV, 0).x;//텍스쳐의 Sample함수를 픽셀셰이더가 아닌 다른곳에서 사용할때는 해당 함수를 써줘야 한다.
